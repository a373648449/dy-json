...
def compress_file(file_name):
    ...
    print(f"File '{file_name}' compressed successfully.")


for source in model_json.keys():
    print(f'fetching {source}')
    fetch_all_models(source)
    compress_file(model_json[source])
    ...
    for p in range(1, num_of_pages + 1):
        ...
        for model in payload['items']:
            ...
            if len(model["modelVersions"]) > 0 and len(model["modelVersions"][0]["files"]) > 0 and "images" in \
                    model["modelVersions"][0].keys() and len(model["modelVersions"][0]["images"]) > 0:
                ...
                EMP_MV_JSON = {"id": 0, "name": "", "createdAt": "", "updatedAt": "",
                               "files": [{"name": "", "hashes": {}}], "images": [{"url": "", "nsfw": "None"}]}
                for mv in model["modelVersions"]:
                    mv_json = {"id": 0, "name": "", "createdAt": "", "updatedAt": "",
                               "files": [{"name": "", "hashes": {}}], "images": [{"url": "", "nsfw": "None"}]}
                    mv_json["id"] = mv["id"]
                ...
                print(
                    f"page {p}/{num_of_pages}: {p * limit_threshold if p * limit_threshold <= total_items else total_items - (p - 1) * limit_threshold}/{total_items}")
